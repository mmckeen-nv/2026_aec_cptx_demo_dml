...
description: "Songwriting craft, Suno prompts, and open-source AI music generation (HeartMuLa, AudioCraft/MusicGen)."
tags: [songwriting, music, suno, parody, lyrics, creative, heartmula, audiocraft, musicgen, audiogen, ai-music]
...
## 10. Open-Source AI Music Models

For local/offline music generation beyond Suno, see the reference guides:

- **[HeartMuLa](references/heartmula.md)** — Open-source (Apache-2.0) lyrics+tags → full song generation. 3B/7B models, comparable to Suno. Requires CUDA GPU (8GB+ VRAM). Installation has dependency patches.
- **[AudioCraft/MusicGen](references/audiocraft.md)** — Meta's text-to-music (MusicGen) and text-to-sound (AudioGen). Multiple model sizes (300M–3.3B). Supports melody conditioning, stereo, style transfer. Also includes EnCodec audio codec.

### Quick Decision Guide

| Need | Tool |
|------|------|
| Songs from lyrics + tags (Suno-like) | HeartMuLa |
| Music from text description | AudioCraft MusicGen |
| Sound effects from text | AudioCraft AudioGen |
| Melody-conditioned generation | AudioCraft MusicGen-melody |
| Style transfer from reference audio | AudioCraft MusicGen-style |
| Commercial/cloud generation | Suno (see §6 above) |
