...
description: "Kanban orchestrator playbook + worker pitfalls/edge-cases for Hermes Kanban profiles."
version: 3.0.0
platforms: [linux, macos, windows]
environments: [kanban]
metadata:
  hermes:
    tags: [kanban, multi-agent, orchestration, routing, worker, pitfalls, edge-cases]
...
For detailed worker-side pitfalls, examples, and edge cases (claim lifecycle, completion patterns, blocking, multi-card operations), see **[references/kanban-worker.md](references/kanban-worker.md)**. The lifecycle itself is auto-injected into every worker's system prompt as KANBAN_GUIDANCE; the reference covers deeper scenarios.
