...
- `controlnet-archviz-pipeline.md` — ControlNet + normal map pipeline for
  architectural visualization: render normal/depth from Blender, use as
  conditioning for SDXL txt2img. Includes Blender 5.1 compositor API changes,
  optimal ControlNet parameters, comparison of approaches, and ground-truth
  depth rendering via ShaderNodeCameraData for interior/close-range scenes.
- `blackwell-gpu-setup.md` — ComfyUI on NVIDIA RTX PRO 6000 Blackwell (sm_120):
  PyTorch nightly cu128 requirement, Hermes venv dependency chain, numpy
  version mismatch fix, launch and verification commands.
...
0. **First model load can hang the server if multiple jobs are queued.**
   When a checkpoint loads for the first time after server start (60-120s),
   submitting additional jobs before the first one completes can make ComfyUI
   permanently unresponsive (`/system_stats` stops answering, `/interrupt`
   times out). **Fix:** Submit ONE simple job first (e.g. 8-step txt2img)
   and wait for it to complete before batching. If hung, `comfy --skip-prompt stop`
   + relaunch. Discovered on ComfyUI 0.24.0 with SDXL, June 2026.
...
9b. **`_comment` key in template workflows** — The skill's bundled workflow
    templates contain a top-level `_comment` key for documentation. This key
    must be removed before submission to `/api/prompt` — ComfyUI expects all
    top-level keys to be numeric node IDs. The `run_workflow.py` script handles
    this automatically, but if you build payloads manually, strip it:
...
12. **img2img input resolution matters** — SDXL works best at 1024px on
    the long edge (e.g. 1024×576 for 16:9). Feeding a full 1920×1080 image
    directly to the VAEEncode node can cause the server to hang or run
    extremely slowly. Always resize input images to SDXL-friendly resolution
    before submitting. Use PIL `Image.resize((1024, 576), Image.LANCZOS)`.

13. **ControlNet model path uses backslashes on Windows** — the
    `ControlNetLoader` node requires the path exactly as reported by the
    `/api/object_info/ControlNetLoader` endpoint. On Windows this uses
    backslashes (e.g. `controlnet-depth-sdxl-1.0\diffusion_pytorch_model.safetensors`).
    Using forward slashes causes `value_not_in_list` errors. Always query the
    API first:
    ```python
    r = requests.get("http://127.0.0.1:8188/api/object_info/ControlNetLoader")
    paths = r.json()["ControlNetLoader"]["input"]["required"]["control_net_name"][0]
    ```

14. **ControlNet depth + img2img: same image = bad, separate images = good** —
    Using the SAME image as both ControlNet input and VAEEncode (img2img) source
    causes double conditioning that fights itself. The ControlNet locks the
    structure so firmly that the denoise has nothing to transform. Result:
    output is nearly identical to the raw input. HOWEVER, using SEPARATE images
    — a beauty render for VAEEncode (img2img latent init) and a ground-truth
    depth map for ControlNet conditioning — works well. The depth map
    constrains the 3D form while the denoise transforms the appearance
    (atmosphere, texture, lighting). This dual-image approach (proven June
    2026 on VP studio interiors) produces results that are neither flat
    (like plain img2img) nor distorted (like pure txt2img). CRITICAL: denoise 0.42-0.50 produces visually identical images across
 seeds (proven June 2026 — user called them "all the exact same"). Must
 use denoise 0.65-0.70 with CN strength 0.72-0.78, PLUS per-seed prompt
 variation (different time-of-day, weather, or scene mood per seed) for
 real visual diversity. The depth map locks geometry; the high denoise
 gives SDXL room to paint atmosphere, texture, and lighting.

15. **ControlNet depth needs a real depth map** — Feeding an RGB render to
    ControlNet depth produces inconsistent results because the model interprets
    color channels as depth values. For best results, render a proper grayscale
    depth map (white=near, black=far) from your 3D scene, or use a depth
    estimation preprocessor (MiDaS/ZoeDepth from `comfyui-controlnet-aux`).

16. **ControlNet strength vs photorealism tradeoff** — Higher ControlNet
    strength preserves form but kills photorealism. At 0.90 strength, SDXL
    outputs look like technical illustrations (4/10 realism). At 0.65, realism
    is good (7.5/10) but complex shapes drift. Sweet spot is 0.75–0.80 with a
    proper depth map. For architecture, always test 3+ strength values.

17. **`comfyui-controlnet-aux` install may fail on some versions** — The package
    name varies: try `comfyui-controlnet-aux`, `comfyui_controlnet_aux`, or
    the git URL `https://github.com/Fannovel16/comfyui_controlnet_aux`. The
    `comfy node install` command can time out (30s+) while scanning the custom
    node registry. If install fails, clone directly into `custom_nodes/`:
    ```bash
