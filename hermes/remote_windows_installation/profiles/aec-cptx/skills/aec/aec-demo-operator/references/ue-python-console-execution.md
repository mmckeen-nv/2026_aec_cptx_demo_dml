...
| Material constant color | `MaterialExpressionConstant` with `.R/.G/.B` | `.R/.G/.B` don't exist | Use `MaterialExpressionConstant3Vector` with `.set_editor_property("constant", unreal.Color(r,g,b,a))` |
| Material node connect | `connect_material_property(node, output, target, input)` — 4 args | Takes 3 args max in 5.8 | Use `connect_material_property(node, output, material, property_name)` — connect to material property directly, not to another node's input |
| Physics simulation | `mesh_comp.set_editor_property("simulate_physics", True)` | Property not found on StaticMeshComponent | Physics must be enabled in the actor's collision properties or via Blueprint, not via `set_editor_property` in editor Python |
| ExponentialHeightFog | `fog.get_editor_property("fog_component")` | Property name different | Use `fog.component` or check `unreal.SystemLibrary.dump_slot()` for correct property names |
| set_actor_location kwargs | `set_actor_location(pos, False)` — 2 args | Requires `sweep` AND `teleport` | `actor.set_actor_location(pos, sweep=False, teleport=False)` — both are required keyword args |
