---
name: claude-code
description: "Delegate coding to external AI agent CLIs: Claude Code, OpenAI Codex, or OpenCode."
version: 2.2.0
...
metadata:
  hermes:
    tags: [Coding-Agent, Claude, Anthropic, Code-Review, Refactoring, PTY, Automation, Codex, OpenAI, OpenCode]
...
This skill primarily covers Claude Code, but the same orchestration patterns (PTY session, progress polling, result extraction) apply to other AI coding CLIs:

- **[OpenAI Codex CLI](references/codex-cli.md)** — `npm install -g @openai/codex`. Autonomous coding agent from OpenAI. Supports `full-auto` mode, multi-provider backends, and project-scoped instructions via `AGENTS.md`.
- **[OpenCode CLI](references/opencode-cli.md)** — Provider-agnostic open-source coding agent with TUI. Supports any OpenAI-compatible endpoint. Good for long-running sessions with progress checks.
